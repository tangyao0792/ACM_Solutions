...
int main()
{
    ...
    while (~scanf("%d", &hull.n) && hull.n){
        ...
        while(q--)
        {
            ...
            for(int i=0;i<cnt;i++)
            {
                double v=hull.volume(p0,
        hull.P[hull.F[i].a],hull.P[hull.F[i].b],hull.P[hull.F[i].c])/6.0;
                double s=hull.area(hull.P[hull.F[i].a],hull.P[hull.F[i].b],hull.P[hull.F[i].c])/2.0;
                double d=3*v/s;
                ans=min(ans,d);
            }
            ans*=10000.0;
            ans=double(int(ans+0.5));
            ans/=10000.0;
            printf("%.4f\n",ans);
        }
    }
